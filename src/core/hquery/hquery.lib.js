...
/**
 * Represents the HQuery class for working with DOM elements.
 */
class HQuery {
	/**
	 * Create a new HQuery instance.
	 ...
	 * @returns {HQuery} A new HQuery instance for the found element.
	 */
	find(selector) {
		const element = new HQuery(this.element.querySelector(selector))
	 ...
	 * @param {string} selector - A CSS selector string to search for within the selected element.
	 * @returns {HQuery[]} An array of new HQuery instances for the found elements.
	 */
	findAll(selector) {
		const elements = this.element.querySelectorAll(selector)
		return Array.from(elements).map(element => new HQuery(element))
	}
	 ...
	 * @returns {HQuery} The current HQuery instance for chaining.
	 */
	append(childElement) {
	 ...
	 * @returns {HQuery|string} The current HQuery instance for chaining when setting HTML content, or the current inner HTML when getting.
	 */
	html(htmlContent) {
	 ...
	 * @returns {HQuery|string} The current HQuery instance for chaining when setting text content, or the current text content when getting.
	 */
	text(textContent) {
	 ...
	 * @return {string|HQuery} - If newValue is provided, returns the HQuery instance. Otherwise, returns the current value of the input element.
	 */
	value(newValue) {
	 ...
	 * @returns {HQuery|string} The current HQuery instance for chaining (if setting) or the attribute value (if getting).
	 */
	 ...
	 * @return {HQuery} - Returns the HQuery instance.
	 */
	removeAttr(attrName) {
...
/**
 * Create a new HQuery instance for the given selector.
 * @param {string|HTMLElement} selector - A CSS selector string or an HTMLElement.
 * @returns {HQuery} A new HQuery instance for the given selector.
 */
