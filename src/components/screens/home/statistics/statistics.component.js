...
export class Statistics extends ChildComponent {
	...
	fetchData() {
		this.statisticService.main(data => {
			...
			const statisticsItemsElement = $H(this.element).find('#statistics-items')
			statisticsItemsElement.text('')

			const circleChartElement = $H(this.element).find('#circle-chart')
	...
	render() {
		if (this.store.user) {
			$H(this.element).append(new Loader().render())
