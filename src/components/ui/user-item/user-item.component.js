...
export class UserItem extends ChildComponent {
	...
	update({ avatarPath, name }) {
		if (avatarPath && name) {
			$H(this.element).find('img').attr('src', avatarPath).attr('alt', name)

			$H(this.element).find('span').text(name)
		}
	}
	...
	render() {
		...
		$H(this.element).click(this.onClick || this.#preventDefault.bind(this))
