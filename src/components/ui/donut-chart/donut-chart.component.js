...
export class DonutChart extends ChildComponent {
	 ...
	 */
	#createSvgElement() {
		const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg') // создание svg
	...
	#createPathElement(slice, pathData) {
		// pathData - те самые теги <path></path> у svg, служат для отрисовки
	...
	#createSvgPathElements(g) {
		...
		this.data.forEach(slice => {
			const percentage = (slice.value / totalValue) * 100 // узнаём процентное соотношение между двумя сегментами
	 ...
	 */
	#getSvg() {
		// генерирование svg элемента
		const svg = this.#createSvgElement()
		const g = this.#createSvgGroupElement()
		this.#createSvgPathElements(g)
		svg.appendChild(g) // добавляем группу уже в svg
	...
	render() {
		...
		$H(this.element).append(this.#getSvg())
